...
class Gasto {
  final String? id;
  final String?
      cajaId; // para compatibilidad si luego quieres asociarlo a una caja
  ...
  factory Gasto.fromJson(Map<String, dynamic> json) => Gasto(
        ...
        items: (json['items'] as List? ?? [])
            .map((e) => GastoItem.fromJson(Map<String, dynamic>.from(e)))
            .toList(),
        pagos: Map<String, double>.from((json['pagos'] as Map? ?? {})
  ...
  Map<String, dynamic> toFirestore() {
    final itemsNorm = items
        .map((e) => {
              ...
              if (e.categoriaId != null && e.categoriaId!.trim().isNotEmpty)
                'categoriaId': e.categoriaId,
            })
        .toList();

    final pagosNorm = {
      for (final e in pagos.entries) e.key: _fromCents(_toCents(e.value))
    };
  ...
  static Gasto fromFirestore(String id, Map<String, dynamic> data) {
    return Gasto.fromJson({
      'id': id,
      ...data,
      'fecha': (data['fecha'] is DateTime)
          ? (data['fecha'] as DateTime).toIso8601String()
          : data['fecha']
    });
  }
}

String encodeGastos(List<Gasto> list) =>
    jsonEncode(list.map((e) => e.toJson()).toList());

List<Gasto> decodeGastos(String jsonStr) => (jsonDecode(jsonStr) as List)
