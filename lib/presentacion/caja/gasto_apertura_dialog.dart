...
Future<Gasto?> showGastoInsumosAperturaDialog(BuildContext context, Caja cajaActiva) async {
  ...
  final recetaGastoXCajaSnap = await recetasCol.where('nombre', isEqualTo: 'Gasto x Caja').limit(1).get();
  final recetaSalsaSnap = await recetasCol.where('nombre', isEqualTo: 'Salsa de ajo').limit(1).get();

  final recetaGastoXCaja = recetaGastoXCajaSnap.docs.isNotEmpty ? recetaGastoXCajaSnap.docs.first : null;
  final recetaSalsa = recetaSalsaSnap.docs.isNotEmpty ? recetaSalsaSnap.docs.first : null;
  ...
  if (_recetaSalsaResolved == null) {
    try {
      ...
      for (final r in allRec.docs) {
        final nombre = (r.data()['nombre'] ?? '').toString().trim().toLowerCase();
        if (nombre == targetNorm) {
          _recetaSalsaResolved = r;
          break;
        }
        // comparar sin tildes/puntuación
        String normalize(String s) => s.trim().toLowerCase().replaceAll(RegExp(r"[^a-z0-9 ]+"), ' ').replaceAll(RegExp(r"\s+"), ' ').trim();
  ...
  if (recetaGastoXCaja != null) {
    final insumos = List<dynamic>.from(recetaGastoXCaja.data()['insumos'] ?? []);
    for (final ins in insumos) {
      ...
      final possibleId = (ins['id'] ?? ins['insumoId'] ?? ins['insumo_id'])?.toString();
  ...
  if (!normalized.containsKey(salsaNorm)) {
    normalized[salsaNorm] = {'display': salsaDisplay, 'id': recetaSalsaFinal?.id};
  } else {
    // si ya existe pero recetaSalsa tiene id, preferirla
    if (recetaSalsaFinal?.id != null) normalized[salsaNorm]?['id'] = recetaSalsaFinal!.id;
  }
      ...
      child: SizedBox(
        ...
        child: ConstrainedBox(
                ...
                child: Row(
                  children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            ...
                            style: TextStyle(
                              fontSize: 12,
                              color: Theme.of(ctx).colorScheme.onPrimary.withOpacity(0.9),
                            ),
                          ),
                        ],
                      ),
                    ...
                    IconButton(
                      ...
                      icon: Icon(Icons.close_rounded, color: Theme.of(ctx).colorScheme.onPrimary),
                      tooltip: 'Cerrar',
                      ...
                      children: [
                        ...
                        ValueListenableBuilder<bool>(
                          ...
                          builder: (context, val, _) => AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              color: val 
                                ? Theme.of(ctx).colorScheme.secondaryContainer.withOpacity(0.3)
                                : Theme.of(ctx).colorScheme.surfaceVariant.withOpacity(0.2),
                              borderRadius: BorderRadius.circular(12),
                              border: Border.all(
                                color: val
                                  ? Theme.of(ctx).colorScheme.secondary.withOpacity(0.4)
                                      ...
                                      children: [
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            ...
                                            boxShadow: [
                                              BoxShadow(
                                                color: Colors.black.withOpacity(val ? 0.15 : 0.12),
                                                blurRadius: val ? 4 : 3,
                                  ...
                                  ValueListenableBuilder<int>(
                                    ...
                                    builder: (context, v2, _) => Container(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          Material(
                                            color: Theme.of(ctx).colorScheme.errorContainer.withOpacity(0.5),
                                            borderRadius: BorderRadius.circular(8),
                                            child: InkWell(
                                              onTap: () => salsaRecetas.value = (salsaRecetas.value > 1) ? salsaRecetas.value - 1 : 1,
                                              borderRadius: BorderRadius.circular(8),
                                              child: Container(
                                                width: 40,
                                                height: 40,
                                          ...
                                          Material(
                                            ...
                                            borderRadius: BorderRadius.circular(8),
                                            child: InkWell(
                                              onTap: () => salsaRecetas.value = salsaRecetas.value + 1,
                                              borderRadius: BorderRadius.circular(8),
                                              child: Container(
                                                width: 40,
                                                height: 40,
          ...
          for (final entry in controllers.entries) {
            ...
            final qty = double.tryParse(txt.replaceAll(',', '.')) ?? 0.0;
            if (qty <= 0) continue;

            // Normalizar para buscar id en el mapa
            final norm = displayKey.trim().toLowerCase();
            final mapped = normalized[norm];
            final itemId = mapped != null ? (mapped['id'] ?? displayKey) : displayKey;

            items.add(GastoItem(id: itemId, nombre: displayKey, precio: 0.0, cantidad: qty));
          }
          ...
          if (sePreparoSalsa.value) {
            final salsaId = recetaSalsaFinal?.id ?? 'Salsa de ajo';
            double salsaCantidad = salsaRecetas.value.toDouble();
            try {
              final data = recetaSalsaFinal?.data() as Map<String, dynamic>?;
              if (data != null) {
                final potesPorReceta = (data['potesPorReceta'] ?? data['rinde'] ?? data['porciones'] ?? data['rendimiento']) as num?;
                if (potesPorReceta != null && potesPorReceta > 0) {
                  salsaCantidad = salsaRecetas.value * potesPorReceta.toDouble();
                }
              }
            } catch (_) {}
            items.add(GastoItem(id: salsaId, nombre: 'Salsa de ajo', precio: 0.0, cantidad: salsaCantidad));
          }
          ...
          if (sePreparoSalsa.value) {
            ...
            try {
              final data = recetaSalsaFinal?.data() as Map<String, dynamic>?;
              final insumosReceta = data != null ? List<dynamic>.from(data['insumos'] ?? []) : <dynamic>[];
              ...
              for (final ins in insumosReceta) {
                final nombreInsumo = (ins['nombre'] ?? '').toString();
                if (nombreInsumo.trim().isEmpty) continue;
                final cantidadPorReceta = parseCantidad(ins['cantidad']);
                final cantidadTotal = cantidadPorReceta * salsaRecetas.value;
                ...
                expanded.add(GastoItem(id: insId, nombre: nombreInsumo, precio: 0.0, cantidad: cantidadTotal));
              }
              if (expanded.isNotEmpty) {
                // Debug: imprimir lista expandida antes de descontar
                try {
                  final detallesExpanded = expanded.map((e) => '${e.nombre} (id=${e.id}): ${e.cantidad}').join('\n');
                  print('GastoApertura: Expanded insumos:\n$detallesExpanded');
                } catch (_) {}
                // Mostrar detalle de lo que se va a descontar (para confirmar decimales)
                final detalle = expanded.map((e) => '${e.nombre}: ${e.cantidad.toString()}').join('\n');
                if (mainScaffoldContext != null) {
                  mostrarNotificacionElegante(mainScaffoldContext!, '''Descontando:\
${detalle}''', messengerKey: principalMessengerKey);
                }
                final reporte = await almacenService.descontarInsumosPorGastoConReporte(expanded);
                if (mainScaffoldContext != null) {
                  final ok = reporte.where((r) => r['found'] == true).toList();
                  final nok = reporte.where((r) => r['found'] != true).toList();
                  final sb = StringBuffer();
                  if (ok.isNotEmpty) {
                    sb.writeln('Descontados:');
                    for (final r in ok) {
                      sb.writeln('- ${r['nombre']}: ${r['descontado']} (antes ${r['antes']} → ${r['despues']})');
                    }
                  }
                  if (nok.isNotEmpty) {
                    sb.writeln('\nNo encontrados:');
                  ...
                  mostrarNotificacionElegante(mainScaffoldContext!, 'Se aplicaron descuentos. Ver detalles.', messengerKey: principalMessengerKey);

                  // Imprimir en consola para depuración
                  try {
                    print('Reporte descuento Salsa: ${sb.toString()}');
                  } catch (_) {}
                  ...
                  if (Navigator.canPop(ctx)) {
                    showDialog<void>(
                      ...
                      builder: (dctx) => AlertDialog(
                        title: const Text('Detalle del descuento aplicado'),
              ...
              if (mainScaffoldContext != null) {
                mostrarNotificacionElegante(mainScaffoldContext!, 'Error al descontar insumos de Salsa: $e', esError: true, messengerKey: principalMessengerKey);
              }
            }
          }
          ...
          if (items.isEmpty) {
            if (mainScaffoldContext != null) {
              mostrarNotificacionElegante(mainScaffoldContext!, 'Debe ingresar al menos un insumo con cantidad o marcar que se preparó salsa.', esError: true, messengerKey: principalMessengerKey);
            }
            return;
          }
